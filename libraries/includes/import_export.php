...
if ($importForm -> isSubmitted()) {
    try {
        ...
        if ($importType == "anything") {
        	...
        	if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
				$excess_elements = sizeof($import_export_types) - 1; // minus the 'anything' plus the employees
        	} else { #cpp#else
        	...
        	if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
        		if ($importType == "users") {
        			$log2 = $importer -> import("employees");
        			//pr($log2);
        			$log["success"] = array_merge($log["success"], $log2["success"]);
        			$log["failure"] = array_merge($log["failure"], $log2["failure"]);
        		}
