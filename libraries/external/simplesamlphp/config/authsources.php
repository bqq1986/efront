...
require_once(__DIR__."/efront-connect.php");
$ef = EfrontConnect::getInstance();

//  echo "<pre>";print_r($sys_config);exit;

//end of efront-specifics
// vd($ef::$config);exit;
$config = array(
	...
	'efront-sp' => array(
			'saml:SP',
			'entityID' => $_SERVER['HTTP_HOST'],
			'idp' => $ef::$config['saml_provider'],
			'redirect.validate' => $ef::$config['saml_bool_redirect_validate'],
			'redirect.sign' => $ef::$config['saml_bool_redirect_sign'],
			'discoURL' => NULL,
			'signature.algorithm' => 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256',
	),
);
if (is_file(dirname(__DIR__).'/cert/saml.crt') && is_file(dirname(__DIR__).'/cert/saml.pem')) {
	$config['efront-sp']['privatekey'] = 'saml.pem';
	$config['efront-sp']['certificate'] = 'saml.crt';
}
