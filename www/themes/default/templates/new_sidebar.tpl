...
<body class = "sidebar" >
    ...
    {*********************************}
    {if isset($T_ONLINE_USERS_LIST) && 'online_users'|eF_template_isOptionVisible}
        ...
        <div class = "verticalTab" id = "logout">
            <div class = "tabHeader">
                <table>
                    <tr>
                    	{if $T_FACEBOOK_USER}
                        	<td colspan="2" class="smallIconCell">
                         	<div style="display:none" id="fb-root"></div>
								 <span id="fbLogout" onclick="facebook_logout()"><a class="fb_button fb_button_small"><span class="fb_button_text">{$smarty.const._LOGOUT}</span></a></span>
				 			</td>	
						{else}
	    ...
	    function facebook_logout() {
	    	 FB.getLoginStatus(function(response) {
	    		 if (response.status === 'connected') { 
			    		...
			    		top.location = "{/literal}{$smarty.const.G_SERVERNAME}{literal}index.php?logout=true"; 
		        	});
	    		} else {
	    			top.location = "{/literal}{$smarty.const.G_SERVERNAME}{literal}index.php?logout=true"; 
		    	}
