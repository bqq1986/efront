    ...
    {foreach name = outerc from = $T_SEARCH_RESULTS_CURRENT_LESSON key= clesson_id item = clesson_results}
        ...
        <div class="resultsTitle">
            {if $cresult.table_name == 'news'}
				<a href = "{$smarty.session.s_type}.php?ctg=news&view={$cresult.id}&popup=1" target = "POPUP_FRAME" onClick = "eF_js_showDivPopup(event, '{$smarty.const._ANNOUNCEMENT}', 1);">
...
{if !empty($T_SEARCH_RESULTS_LESSONS)}
	...
	{foreach name = outer from = $T_SEARCH_RESULTS_LESSONS key= lesson_id item = lesson_results}
        ...
        <div class="resultsTitle">
            ...
            {else}
                <a href = "{$smarty.session.s_type}.php?ctg=news&view={$result.id}&popup=1" target = "POPUP_FRAME" onClick = "eF_js_showDivPopup(event, '{$smarty.const._ANNOUNCEMENT}', 1);"><img src="images/16x16/announcements.png" border="0" align="top"/>&nbsp;{$result.name}</a>
			{/if}
...
{capture name = 't_users_search_results_code'}
    <table width="100%">
            ...
            <tr>
                ...
                <a title="#filter:login-{$item.login}#" target="POPUP_FRAME" onclick="eF_js_showDivPopup(event, '{$item.login}', 2)" href="{$smarty.server.PHP_SELF}?ctg=messages&add=1&recipient={$item.login}&popup=1"><img border="0" alt="{$smarty.const._SENDPERSONALMESSAGE}" title="{$smarty.const._SENDPERSONALMESSAGE}" src="images/16x16/mail.png"/></a>
