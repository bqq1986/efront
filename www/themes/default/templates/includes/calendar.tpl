...
<!--ajax:calendarTable-->
		...
		{foreach name = 'calendar_events_list' key = id item = event from = $T_DATA_SOURCE}
			...
			{if ($smarty.session.s_type == 'administrator' || $smarty.session.s_login == $event.users_LOGIN) && $_change_}
					<a href = "{$smarty.server.PHP_SELF}?ctg=calendar&edit={$id}&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._EDITEVENT}', 2)" target = "POPUP_FRAME"><img src = "images/16x16/edit.png" alt = "{$smarty.const._EDITEVENT}" title = "{$smarty.const._EDITEVENT}" class = "hande"></a>
		...
		{capture name = 't_calendar_page_code'}
			...
			<div class = "headerTools">
				<span>
					...
					<a href = "{$smarty.server.PHP_SELF}?ctg=calendar&view_calendar={$T_VIEW_CALENDAR}{if $T_SHOW_INTERVAL}&show_interval={$T_SHOW_INTERVAL}{/if}&add=1&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._ADDEVENT}', 2)" target = "POPUP_FRAME">{$smarty.const._ADDEVENT}</a>
				</span>
			</div>
			{/if}
