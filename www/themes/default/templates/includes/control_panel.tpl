...
{else}
			...
			<tr><td class = "moduleCell">
		        {capture name='t_news_code'}
		        	...
		        	{foreach name = 'news_list' item = "item" key = "key" from = $T_NEWS}
		        		<tr><td>{$smarty.foreach.news_list.iteration}. <a title = "{$item.title}" href = "{$smarty.server.PHP_SELF}?ctg=news&view={$item.id}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._ANNOUNCEMENT}', 1);">{$item.title}</a></td>
    ...
    {if $T_COMMENTS && 'comments'|eF_template_isOptionVisible}
		...
		<tr><td class = "moduleCell">
				<table class = "horizontalBlock">
                	<tr><td>
						...
						{if $T_COURSE_USER_REMAINING_STRING}
							{$T_COURSE_USER_REMAINING_STRING}
						{/if}
