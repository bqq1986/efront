...
{else}
	{capture name = 't_employee_jobs'}
		{if $_change_placements_}
			<div class = "headerTools">
				<span>
					...
					<a href="{$smarty.server.PHP_SELF}?ctg=personal&user={$smarty.get.user}&op=org_form&add_placement=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._NEWJOBPLACEMENT}', 2)">{$smarty.const._NEWJOBPLACEMENT}</a>
				</span>
			</div>
		{/if}
		...
		{if $_change_placements_}
				...
				{if $smarty.session.s_type == 'administrator' || in_array($placement.branch_ID, $T_SUPERVISES_BRANCHES)}
					<a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$smarty.get.user}&op=org_form&edit_placement={$placement.job_description_ID}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._EDITJOBPLACEMENT}', 2)"><img class = "handle" src = "images/16x16/edit.png" title = "{$smarty.const._EDIT}" alt = "{$smarty.const._EDIT}" /></a>
