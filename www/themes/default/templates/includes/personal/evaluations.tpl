	...
	{capture name = 't_evaluations_code'}
		...
		<div class = "headerTools">
			<span>
				...
				<a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$smarty.get.user}&op=evaluations&add_evaluation=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._NEWEVALUATION}', 1)">{$smarty.const._NEWEVALUATION}</a>
			</span>
		</div>
		{/if}
