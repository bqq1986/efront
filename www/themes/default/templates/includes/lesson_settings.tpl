                                ...
                                {foreach name = 'users_to_lessons_list' key = 'key' item = 'user' from = $T_ALL_USERS}
                                    ...
                                    {/if}
                                                            ...
                                                            {if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
                                                                ...
                                                                <a href = "{$smarty.server.PHP_SELF}?{if $smarty.session.s_type == "administrator"}ctg=personal&user={$user.login}&op=evaluations{else}ctg=evaluations&user={$user.login}{/if}">
                                                                    <img src="images/16x16/search.png" title="{$smarty.const._SHOWEVALUATIONS}" alt="{$smarty.const._SHOWEVALUATIONS}"/ border="0"></a>
                                                            {/if} {* #cpp#endif *}
...
<!--/ajax:usersTable-->
    ...
    {elseif isset($T_OP) && $T_OP == 'lesson_layout'}
         {capture name = "t_layout_code"}
                    ...
                    {/capture}
					{/if}
                    {if 'news'|eF_template_isOptionVisible}
	                    ...
	                    {capture name = "layout_moduleTimeline"}
	                        <li id = "layoutsecondlist_moduleTimeline">
	                            <table class = "innerTable" style = "width:300px">
	                                <tr class = "handle">
	                                    <th class = "innerTableHeader">
	                                        <img class = "iconTableImage" src = "images/32x32/users.png" alt = "{$smarty.const._LESSONPEOPLE}" title = "{$smarty.const._LESSONPEOPLE}">
	                                        {$smarty.const._LESSONPEOPLE}
	                                    </th>
	                                    <td class = "innerTableHeader" style = "text-align:right"><img src = "images/16x16/{if isset($T_DEFAULT_POSITIONS.visibility.moduleTimeline) && $T_DEFAULT_POSITIONS.visibility.moduleTimeline == 0}navigate_down.png{else}navigate_up.png{/if}" onclick = "toggleVisibility(Element.extend(this).up().up().next().down(), this);"></td></tr>

	                                <tr><td colspan = "2" style = "height:60px;text-align:center;vertical-align:top;{if isset($T_DEFAULT_POSITIONS.visibility.moduleTimeline) && $T_DEFAULT_POSITIONS.visibility.moduleTimeline == 0}display:none;{/if}"><img src = "images/others/lesson_people_thumbnail.png"></td></tr>
	                            </table>
	                        </li>
	                    {/capture}
	                    {/if}
