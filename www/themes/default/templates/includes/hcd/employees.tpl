...
<!--ajax:usersTable-->
	...
	<table style = "width:100%" class = "sortedTable" sortBy = "0" size = "{$T_TABLE_SIZE}" id = "usersTable" useAjax = "1" branchFilter="true" jobFilter="{$T_JOBS_FILTER}" activeFilter = 1 rowsPerPage = "{$smarty.const.G_DEFAULT_TABLE_SIZE}" url = "{$smarty.server.PHP_SELF}?ctg=users&">
		<tr class = "topTitle">
		...
		<tr id="row_{$user.login}" class = "{cycle values = "oddRowColor, evenRowColor"} {if !$user.active}deactivatedTableElement{/if}">
			<td id="column_{$user.login}">
				...
				<a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$user.login}&op=profile" class = "{if $user.active == 1}editLink{/if} {if 'tooltip'|eF_template_isOptionVisible}info{/if}" url = "ask_information.php?users_LOGIN={$user.login}&type=user">#filter:login-{$user.login}#</a>
				{else}
...
{/if}
	...
	<tr>
		...
		{else}
			...
			{if 'show_unassigned_users_to_supervisors'|eF_template_isOptionVisible}
			<div class="tabber">
				<div class="tabbertab">
					...
					<tr id="row_{$user.login}" class = "{cycle values = "oddRowColor, evenRowColor"} {if !$user.active}deactivatedTableElement{/if}">
						...
						<td class = "centerAlign">
							{if $user.login != $smarty.session.s_login && $user.user_type != 'administrator'}
								<a href="{$smarty.session.s_type}.php?ctg=personal&user={$user.login}&op=profile&op=status&print_preview=1&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._EMPLOYEEFORMPRINTPREVIEW}', 2)" target = "POPUP_FRAME"><img src='images/16x16/printer.png' title=  '{$smarty.const._PRINTPREVIEW}' alt = '{$smarty.const._PRINTPREVIEW}' border='0' /></a>
							{else}
