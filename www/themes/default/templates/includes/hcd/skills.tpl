		...
		<table class = "formElements">
			...
			<tr><td class = "labelCell">{$T_SKILLS_FORM.category.label}:&nbsp;</td>
				<td class = "elementCell">
					...
					{if $_change_}
						<a href="{$smarty.session.s_type}.php?ctg=module_hcd&op=skill_cat&add_skill_cat=1&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._ADDSKILLCATEGORY}', 2)" target = "POPUP_FRAME"><img src='images/16x16/add.png' title=  '{$smarty.const._ADDSKILLCATEGORY}' alt = '{$smarty.const._ADDSKILLCATEGORY}' border='0' /></a>
				  		<a id = "edit_skill_cat" href="{$smarty.session.s_type}.php?ctg=module_hcd&op=skill_cat&edit_skill_cat={$T_DEFAULT_CATEGORY}&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._EDITSKILLCATEGORY}', 2)" target = "POPUP_FRAME"  {if $T_DEFAULT_CATEGORY == ""}style="visibility:hidden"{/if}><img src='images/16x16/edit.png' title=  '{$smarty.const._EDITSKILLCATEGORY}' alt = '{$smarty.const._EDITSKILLCATEGORY}' border='0' /></a>
		...
		<div class = "headerTools">
			{if $_change_}
				<span>
					...
					<a href = "{$smarty.server.PHP_SELF}?ctg=module_hcd&op=skills&add_skill=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._NEWSKILL}', 2)">{$smarty.const._NEWSKILL}</a>
				</span>
			{/if}
		</div>
		{/if}
