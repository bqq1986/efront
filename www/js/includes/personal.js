...
function changeStatus() {
    if (__initStatus != $('inputStatusText').value) {
        ...
        ajaxRequest($('statusTextProgressImg'), url, parameters, onChangeStatus);
...
function onChangeStatus(el, response) {
    if (response.evalJSON(true).redirect) {
    	window.location = response.evalJSON(true).redirect
    }
}
