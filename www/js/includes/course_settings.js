...
function showEdit(id) {
	if (id == 0) { 			
		...
		$('schedule_dates_'+id).hide();
		$('remove_schedule_link_'+id).show();
		$('deadline_dates_form_'+id).show();
		$('period_submit_'+id).show();
		$('toggle_way_'+id).show();
		
	}
}

function switchWay(id) {
	$('schedule_dates_form_'+id).toggle();
	$('deadline_dates_form_'+id).toggle();
	$('set_schedules_link_'+id).toggle();
	$('period_submit_'+id).toggle();	
	if ($('toggle_way_'+id).hasClassName('sprite16-calendar')) {
		setImageSrc($('toggle_way_'+id), 16, 'calendar_selection_week.png');
		$('toggle_way_'+id).setAttribute('alt', switchperiodmode);
		$('toggle_way_'+id).setAttribute('title', switchperiodmode);
	} else {
		setImageSrc($('toggle_way_'+id), 16, 'calendar.png');
		$('toggle_way_'+id).setAttribute('alt', switchcalendarmode);
		$('toggle_way_'+id).setAttribute('title', switchcalendarmode);
	}
}
...
function hideEdit(id) {
	...
	if (id != 0) {
		$('deadline_dates_form_'+id).hide();
		$('period_submit_'+id).hide();
		$('toggle_way_'+id).hide();		
	}
}
...
function setPeriod(el, id) {
	parameters = {method: 'get', set_period: id};	
	var url    = location.toString();
	$('deadline_dates_form_'+id).select('select').each(function (s) {url+='&'+s.name+'='+s.options[s.selectedIndex].value;});
	ajaxRequest(el, url, parameters, onSetPeriod);
}
function onSetPeriod(el, response) {
	id = el.id.match(/period_submit_(\d+)/)[1];
	$('schedule_dates_'+id).update(response);
	hideEdit(id);
	setImageSrc($('add_schedule_link_'+id).down(), 16, 'edit');
	$('add_schedule_link_'+id).down().next().show();
