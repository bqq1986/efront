function activate(el, action) {
	...
	el.down().down().className.match(/inactiveImage/) ? parameters = Object.extend(parameters, {activate:action}) : parameters = Object.extend(parameters, {deactivate:action}) ;
...
function onActivate(el, response) {
	...
	if (el.down().down().className.match(/inactiveImage/)) {
		el.down().down().removeClassName('inactiveImage');
	} else {
		el.down().down().addClassName('inactiveImage');
	}
