...
{capture name = "t_module_flashcard_cards_list_code"}
        <table class = "QuestionsListTable sortedTable" id = "flashcardsCardTable">
            ...
            <tr class = "{cycle name = "main_cycle" values="oddRowColor, evenRowColor"}">
            	...
            	<td class = "centerAlign">
					<a href = "{$smarty.server.PHP_SELF}?ctg=tests&show_question={$item->question.id}&popup=1" target = "POPUP_FRAME" onclick="eF_js_showDivPopup(event, '{$smarty.const._PREVIEW}',2)"><img src = "{$T_MODULE_FLASHCARDS_BASELINK}images/search.png" alt="{$smarty.const._PREVIEW}" title="{$smarty.const._PREVIEW}" border = "0" style = "vertical-align:middle" /></a>
...
//				$('flip_card').toggle();
	...
	{else}
		...
		<table class = "sortedTable" width="100%">
			...
			{foreach name = "flashdecks_list" item = "deck" key = "key" from = $T_FLASHCARDS_DECKS}
			<tr class = "{cycle name = $key values = "oddRowColor,evenRowColor"}">
				<td><span style="display:none">{$T_FLASHCARDS_DECKSNAMES.$key.name}</span><a href="{$T_MODULE_FLASHCARDS_BASEURL}&view_deck={$key}&popup=1"  onclick = "eF_js_showDivPopup(event, '{$smarty.const._FLASHCARDS_FLASHCARDS}', 3)" target="POPUP_FRAME">{$T_FLASHCARDS_DECKSNAMES.$key.name}</a></td>
