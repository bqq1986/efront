...
{capture name = 't_inner_table_code'}
    <table>
        {section name = 'faq_list' loop = $T_FAQ_INNERTABLE max = 10}
            <tr><td>{$smarty.section.faq_list.iteration}.</td><td><a href = "{$T_FAQ_MODULE_BASEURL}#{$smarty.section.faq_list.iteration}">{$T_FAQ_INNERTABLE[faq_list].question|eF_truncate:80}</a></td></tr>
            <tr><td></td><td><i>{$T_FAQ_INNERTABLE[faq_list].answer|eF_truncate:80}</i></td></tr>
        {sectionelse}
