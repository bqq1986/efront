    ...
    {capture name = 't_rss_code'}
		<div class = "headerTools">
			<span>
				...
				<a href = "{$T_RSS_MODULE_BASEURL}&add_feed=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._RSS_ADDFEED}', 0)">{$smarty.const._RSS_ADDFEED}</a>
			</span>
		</div>
    	...
    	{foreach name = 'feed_loop' key = "id" item = "feed" from = $T_RSS_FEEDS}
    		...
    		{/if}
    			...
    			{if $T_RSS_USERTYPE == 'administrator' || $feed.lessons_ID > 0}
    				<a href = "{$T_RSS_MODULE_BASEURL}&edit_feed={$feed.id}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._RSS_EDITFEED}', 0)"><img src = "{$T_RSS_MODULE_BASELINK}images/edit.png" alt = "{$smarty.const._EDIT}" title = "{$smarty.const._EDIT}" border = "0"></a>
    ...
    {capture name = "t_rss_provider_code"}
		<div class = "headerTools">
			...
			<span>
				...
				<a href = "{$T_RSS_MODULE_BASEURL}&add_feed_provider=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._RSS_PROVIDEFEED}', 0)">{$smarty.const._RSS_PROVIDEFEED}</a>
			</span>
			{/if}
			<span>
				...
				<a href = "{$T_RSS_MODULE_BASEURL}&add_feed_provider=1&lesson=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._RSS_PROVIDELESSONFEED}', 1)">{$smarty.const._RSS_PROVIDELESSONFEED}</a>
			</span>
		</div>
        <table class = "sortedTable" style = "width:100%">
        	<tr>
    	...
    	{foreach name = 'feed_loop' key = "id" item = "feed" from = $T_RSS_PROVIDED_FEEDS}
    		...
    		{/if}
    			...
    			<td class = "centerAlign">
    				...
    				<a href = "{$T_RSS_MODULE_BASEURL}&edit_feed_provider={$feed.id}{if $feed.mode == 'lesson'}&lesson=1{/if}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._RSS_EDITFEED}', 0)"><img src = "{$T_RSS_MODULE_BASELINK}images/edit.png" alt = "{$smarty.const._EDIT}" title = "{$smarty.const._EDIT}"></a>
