            ...
            {else}
                {if $T_BBB_CURRENTLESSONTYPE == "student"}
                    ...
                    {elseif $meeting.status == "1" }  
                    	<a href = "{$meeting.joiningUrl}" target="_blank" class = "editLink"><img border = "0" src = "{$T_BBB_MODULE_BASELINK}images/server_client_exchange.png" title = "{$smarty.const._BBBJOINMEETING}" alt = "{$smarty.const._BBBJOINMEETING}" /></a>
                    {/if}
                {else}
