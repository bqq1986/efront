...
{capture name = 't_gradebook_code'}
	...
	<div class = "headerTools">
		<span>
			...
			<a href="{$T_GRADEBOOK_BASEURL}&add_range=1&popup=1" target="POPUP_FRAME" onclick="eF_js_showDivPopup(event, '{$smarty.const._GRADEBOOK_ADD_RANGE}', 0)">{$smarty.const._GRADEBOOK_ADD_RANGE}</a>
		</span>
	</div>
...
{foreach name = 'ranges_loop' key = "id" item = "range" from = $T_GRADEBOOK_RANGES}
	<tr id="row_{$range.id}" class="{cycle values = "oddRowColor, evenRowColor"}">
		...
		<td class="centerAlign">
			<a href="{$T_GRADEBOOK_BASEURL}&edit_range={$range.id}&popup=1" target="POPUP_FRAME" onclick="eF_js_showDivPopup(event, '{$smarty.const._GRADEBOOK_EDIT_RANGE}', 0)"><img src="{$T_GRADEBOOK_BASELINK}images/edit.png" alt="{$smarty.const._EDIT}" title="{$smarty.const._EDIT}" border="0"></a>
